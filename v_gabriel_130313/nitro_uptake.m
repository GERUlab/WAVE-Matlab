...
%Define minimum concentration that will remain in the soil. Below this
%concentration no (passive and active) transport will occur
%rcsolo=[0 1.8/1000 6.2/1000]; %%Units?? 
rcsolo = [0 10/30/10^5 10/30/10^5];
volume=dx;
...
rorad=plant_uptake_param(1,1);       %%rorad=nitro_param(4);
rdo=plant_uptake_param(1,2);         %%rdo=nitro_param(5);
g= plant_uptake_param(1,3);           %%%g= nitro_param(13);  
%rnmaxp=plant_uptake_param(index,4);  %%rnmaxp=nitro_param(1);
rdens0=plant_uptake_param(1,5);   %%rdens0=nitro_param(2);
alfa_rdens=plant_uptake_param(1,6);  %%alfa_rdens=nitro_param(3);
...
%rnmaxp = NDemand*PLM2*PropFact/10^4; %Maximum uptake of one day
rnmaxp = NDemand*PLM2(ncrop)/10^4;
%rnmaxp = min(rnmaxp, 5/10^5/fraction_plant);    
    if first_time == 1
        first_time =0;
        if isucr 
        else
            %totdem = rnmaxp/100; %Initial value for total demand
        end
    end
...
%Calculate root density ==> should come from crop module.
%[rdens,drza]= calc_rdens(plant_uptake_param,ncs,dx,drz,t);
%[rdens, drza] = calc_rdens2(t,drz,ncrop);
% rdens = rdens/CC; %Upscaling to the column
   ...
   totvr = 0;
    ...
    if tconn > totdem ; %=>Demand can ce fulfilled with convective uptake
        ...
        for sp=2:3;
            for i=1:ncs
        	    ...
        	    und(i,sp)=rdens(i)*rorad*2*pi*diffus_rm(i,sp)*hcsolo(sp)*wco(i)*dx*dt/(rdo*100000);
         	    %und(i,sp)=rdens(i)*rorad*2*pi*diffus_rm(i,sp)*hcsolo(sp)*wco(i)*dx*dt/(rdo);
                %diffus_rm(i,sp)=0;
                %und(i,sp)=0;
        	    tund=tund+und(i,sp);
            end
        ...
        if tund > tpdnup;  %==>Active transport can complete the demand
            %==> redistribute the uptake pattern
            for sp = 2:3;
            und(1:ncs,sp)=und(1:ncs,sp)*tpdnup/tund;
            end
            tund=tpdnup;
        %else :do nothing, calculated value is actual value
        %end
        end
    end
