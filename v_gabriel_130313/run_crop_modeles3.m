function         [crop_result, RootResDay,crop_result2] =... 
                 run_crop_modeles3(CropParams,output_crop,simplant, plant_date, harvest_date,t, Nuptake_time,output_cropNup,PLM2,cropinit,crop_climate)
...
%initialisation parameters, estimated in ReadDSSATCBT.R
    ...
    DMLeaf = cropinit.DWLeaf(ncrop);%CropParams(6);
    DMGreenLeaf = DMLeaf;
...
%*****************************************************************
    ...
    %LE = (1/(1+(delta/gamma) + (ri/re))*( ((delta/gamma)* (Rn-H)) + (LAICov.*rho.air*Cp*VPD)/(gamma*re)))*TotLeafArea * PLM2; 
    %LE = (1/(1+(delta/gamma) + (ri/re))*( ((delta/gamma)* (Rn-H)) + (LAICov.*rho.air*Cp*VPD_nok)/(gamma*re)))*TotLeafArea * PLM2; 
...
%forcingfunction = xlsread('ForcingNuptakeCauli.xls');
%dlmwrite('forcing_cauli.txt', forcingfunction)
forcingfunction = dlmread('forcing_cauli.txt');
forcingfunction =forcingfunction*0.6;
dailydemand = diff(forcingfunction(:,3));
demand = dailydemand(DAP);
StemNDemand = demand *0.2;
LeafNDemand = demand*0.4;
CurdNDemand = demand*0.3;
...
crop_result = [DATE,DAP,GP,RMAINT,MeanDayTemp,Te,TeSum,DVS,TotDM,DMRoot,DMLeaf,LeafArea,DMStem,DMCurd,TotDMLeaf,TotDMStem,TotDMCurd,TotDMRoot,TotDMShoot,TotLeafArea,LAICov,LAI,TotDMGreenLeaf, ...
                        TotDMDeadLeaf,TotRootDepth1,NaN,Transpiration,Evaporation,CumTranspiration,CumEvaporation,LeafStemNDemand,CurdNDemand,RootNDemand,PlantNDemand,TotNDemandLeafStem,TotNDemandCurd, ...
