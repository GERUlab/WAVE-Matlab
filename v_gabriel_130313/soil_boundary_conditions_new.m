...
%end 
%%%%%%%%%%%%%%%
 %[IPBCM,hs,phsa,arel,brel,initial_gwl]=In_Boundary_conditions;
if dt_changed_bc==1 % In case the dt have been changed to fit the limits of the BC (see calcdt)
    dt_changed_bc=0;    
    if next_dt_new_bc==1
        next_dt_new_bc=0;
        [IPBCM,hs,phsa,arel,brel,initial_gwl]=In_Boundary_conditions(wat_bottom_BC_type,theta_table); %TO DO find a way to check original input of BC
        if(IPBCM(i,2)==2 & boco_top_type==1 & boco_top>0)
        %if (BOUNDARY_CONDITIONS_MATRIX(i,2)==2 & boco_top_type==1 & boco_top>0)
        % New user BC is now flux (2) and  different from previous (1)
        % and there is water which has not been infiltrated (boco_top>0);
        % In this case, ponding will appear. 
...
[IPBCM,hs,phsa,arel,brel,initial_gwl]=In_Boundary_conditions(wat_bottom_BC_type,theta_table);
