function [minerm,mineri,rcarbmin,soil_om,carbman,carblit,rnitman,rnitlit,cco2o]=...
          miner_immob(t,dt,temp,ncs,WC,csol,ph,soil_om,wco,dx,cm,cim,soil_parameters,...
         om_param, miner_param,plant_uptake_param);
...
alfa=soil_parameters(:,3);
 ...
 ro=miner_param(1);  %%%ro=nitro_param(6);
 fe=miner_param(2);  %%fe=nitro_param(7);
 fh=miner_param(3);  %%fh=nitro_param(8);
 k_lito=om_param(:,5); %%k_lito=om_param(:,1);
 k_mano=om_param(:,6);  %%k_mano=om_param(:,2);
 k_humo=om_param(:,7);  %% k_humo=om_param(:,3);
%%%%%%%%%%%%%%%%INITIAL OM
 c_manp=soil_om(:,1);
 n_manp=soil_om(:,2);
 c_litp=soil_om(:,3);
...
for i=1:ncs;
 %%   reduction factor for moisture
    ...
    redmoist(i)=((wco(i)-thetaw(i))/(theta1(i)-thetaw(i)));
    elseif wco(i)>= wcs(i) 
    redmoist(i)=0.5;
...
cco2o=(1-fe)*(k_man.*c_manp(i)+k_lit.*c_litp(i))*dt+k_hum.*c_hump(i)*dt;
