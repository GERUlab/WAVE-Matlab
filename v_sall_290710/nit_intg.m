...
ptsnorg = ptsnorg + sum(decnorg)*volume*dt;  %% cumulative profile total (ptsnorg conservé)
ptscorg = ptscorg + sum(deccorg)*volume*dt;  %% cumulative profile total (ptscorg conservé)
...
cberr_norg= dnorg - (tflnorg + ptsnorg); %% (erreur = variation stock -(entrées-sorties))
