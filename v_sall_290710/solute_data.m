function [cm,cim,csol,acsolmo]=soldata (cm,cim,csol,acsolmo,t,dx,tcsolo_ini,wco,wcmo,wcma,wcio,immobile,solute_param1,solute_param2,nsol,ncs,pvela);
...
if t==0
 ...
 %%&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
		...
		tcsolmo(i,j)=tcsolo_ini(i,j)*wcmo(i)/wco(i);   %%% en ML-²
		cim(i,j)=tcsolio(i,j)/(volume*(kd((i),j)*(1-f((i)))*bd((i))+wcio(i))); %% en ML-³
