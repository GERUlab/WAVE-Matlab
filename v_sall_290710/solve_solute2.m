...
A(1,1)= -bb(1,1)-betha3(I)*db(1,1);
B(1,1)= wcmo(1)/dt +ab(1,1)+bb(1,1)+(bd(1)*f(1)*kd(1,j))/dt-betha2(1)*db(1,1)+betha4(1)*cb(1,1);
E(1,1)= csp(j)*(ab(1,1) - betha1(I)*cb(1,1))+ cs(j)*(ab(1,1)-betha1(1)*cb(1,1))+...
...
A(I,1)= -bb(I,1)-betha3(I).*db(I,1);
B(I,1)= (wcmo(I))'./dt +ab(I,1)+bb(I,1)+(bd(I).*f(I).*kd(I,j))/dt-betha2(I).*db(I,1)+betha4(I).*cb(I,1);

D(I-1,1)= -ab(I,1)+betha1(I).*cb(I,1);

E(I,1)= cmp((I-1),j).*(ab(I,1) - betha1(I).*cb(I,1))+ ...
...
dt*pveloh(ncomp)*((pvelah(ncomp)))*(wcmo(ncomp)-wcmob(ncomp))/(16*dx^2);
...
B(ncomp,1)= wcmo(ncomp)/dt +ab(ncomp,1)+bb(ncomp,1)+(bd(ncomp)*f(ncomp)*kd(ncomp,j))/dt -...
    betha2(ncomp)*db(ncomp,1)+betha4(ncomp)*cb(ncomp,1);

D(ncomp-1,1)= -ab(ncomp,1)+betha1(ncomp)*cb(ncomp,1); 

E(ncomp,1)=cmp((ncomp-1),j)*(ab(ncomp,1)-betha1(ncomp)*cb(ncomp,1)) +...
cmp(ncomp,j)*(wcmob(ncomp)/dt-ab(ncomp,1)+(bd(ncomp)*f(ncomp)*kd(ncomp,j)/dt)+...
 ...
 cm(:,j)=(inv(diag(B)+diag(A,1)+diag(D,-1))*E).'; %%
