function[wat_flxsa, wat_flxsah, pvela, pvelah, pveloh,pvelo, wcio, wciob, wcma, wcmah,wcmob, wcmo, wco, wcob,diffus,diffush]=...
         wat_sol(t,wat_flxs, wat_flxsa, immobile, ncs,solute_param1,solute_param2,pvela,WC, wcio, wcma,wcmo, wco,pvelo,...
         diffus,nsol,dx,sol_upper_bc_type);

%wat_sol: prepare data from the water flow module  for the solute module     
...
%k=[1:ncs];
%if lm(k)<=1.0
 %   lm(k)=max(0,(lm(k)-0.139*dx)); % correction for numerical dispersion if necessary
%end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
dm=lm.*abs(pvela');
%%%%%%%%%%%%%%%%%%%%
